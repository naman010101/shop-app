...
import { Plus, Edit2, Trash2, Search, ArrowLeftRight } from 'lucide-react';
import { toast } from 'react-hot-toast';

interface ShopTransferRecord {
  id: string;
  item_name: string;
  quantity: number;
  slip_number: string;
...
export default function ShopTransferPage() {
  ...
  // ── Records state ───────────────────────────────────────────────────────────
  const [records, setRecords] = useState<ShopTransferRecord[]>([]);
  ...
  const [editRecord, setEditRecord] = useState<ShopTransferRecord | null>(null);
  ...
  const [deleteRecord, setDeleteRecord] = useState<ShopTransferRecord | null>(null);
  ...
  const fetchRecords = async () => {
    ...
    try {
      ...
      toast.error('Failed to load shop transfer records.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    setSubmitting(true);
    try {
      await api.post('/warehouse/shop-transfer', {
        item_name: itemName.trim(),
        quantity: parseInt(quantity),
        slip_number: slipNumber.trim(),
        by_person: byPerson.trim(),
      });
      toast.success('Shop stock transfer recorded successfully!');
      resetCreateForm();
      setCreateOpen(false);
      setPage(1);
      fetchRecords();
  ...
  const handleEditClick = (rec: ShopTransferRecord) => {
    setEditRecord(rec);
  ...
  const handleUpdate = async (e: React.FormEvent) => {
    ...
    try {
      await api.put(`/warehouse/shop-transfer/${editRecord.id}`, {
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/warehouse/shop-transfer/${deleteRecord.id}`);
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        ...
        emptyMessage="No shop stock transfer records match your filters."
      />

      {/* ═══════════════════════ CREATE MODAL ═══════════════════════ */}
      <Modal isOpen={createOpen} onClose={() => { setCreateOpen(false); resetCreateForm(); }} title="Add Shop Stock Transfer Entry">
