...
const getAll = async (req, res, next) => {
  try {
    ...
    const [records, total] = await Promise.all([
      ...
      prisma.cashOutflow.count({ where }),
    ]);
...
const create = async (req, res, next) => {
  try {
    ...
    res.status(201).json({ message: 'Cash outflow recorded successfully.', record });
  } catch (error) {
    next(error);
  }
};
...
const update = async (req, res, next) => {
  try {
    ...
    const record = await prisma.cashOutflow.update({
      where: { id },
      data: {
...
const remove = async (req, res, next) => {
  try {
    ...
    const { id } = req.params;
    await prisma.cashOutflow.delete({ where: { id } });
